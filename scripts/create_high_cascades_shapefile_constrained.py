...
def remove_artifacts_and_simplify(gdf, min_area_sqkm=0.08, max_perimeter_area_ratio=None):
    ...
    high_cascades_bioregion = gpd.GeoDataFrame(
        [{
            ...
            'region_code': 'high_cascades',
            'description': f'High Cascades forest ≥{MIN_ELEVATION_FT}ft elevation with ≥{MIN_TREE_COVER_PCT}% tree cover',
...
def validate_bioregion_elevations(bioregion_gdf, original_boundary_gdf):
    ...
    with rasterio.open(ELEVATION_RASTER) as elev_src:
        ...
        if sample_elevations:
            ...
            bins = [0, 1000, 2000, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 7000, 8000, 10000]
            ...
            within_range = np.sum(elevations >= MIN_ELEVATION_FT)
            within_pct = within_range / len(elevations) * 100
            
            print(f"\nConstraint Validation:")
            print(f"  Samples ≥ {MIN_ELEVATION_FT} ft: {within_range}/{len(elevations)} ({within_pct:.1f}%)")
...
def main():
    ...
    high_cascades_bioregion = remove_artifacts_and_simplify(high_cascades_polygons)
    
    if high_cascades_bioregion.empty:
        print("No polygons remain after filtering")
        return
    
    # Elevation validation for High Cascades
    print("\n=== Elevation Validation for High Cascades Bioregion ===")
    validate_bioregion_elevations(high_cascades_bioregion, boundary)
    ...
    summary = {
        ...
        'results': {
            'total_area_sqkm': float(high_cascades_bioregion.area_sqkm.iloc[0]),
    ...
    print(f"\n=== Final Summary ===")
    print(f"High Cascades forest bioregion area: {high_cascades_bioregion.area_sqkm.iloc[0]:.0f} km²")
